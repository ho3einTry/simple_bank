...
func (q *Queries) CreatEntry(ctx context.Context, arg CreatEntryParams) (Entry, error) {
	row := q.queryRow(ctx, q.creatEntryStmt, creatEntry, arg.AccountID, arg.Amount)
	var i Entry
	err := row.Scan(
		&i.ID,
		&i.AccountID,
...
func (q *Queries) ListEntryByAccountId(ctx context.Context, arg ListEntryByAccountIdParams) ([]Entry, error) {
	rows, err := q.query(ctx, q.listEntryByAccountIdStmt, listEntryByAccountId, arg.AccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
